...
async function rebuildCache() {
  ...
  try {
    ...
    if (discovered.length > 0) {
      cache.data = discovered.slice(0, 30);
...
export default async function handler(req, res) {
  ...
  const isFresh = cache.data && (now - cache.timestamp < ONE_DAY);
  const isHealthy = cache.data && cache.data.length >= MIN_PRODUCTS;

  // ✅ Serve fresh & healthy cache
  if (isFresh && isHealthy) {
    return res.status(200).json({ products: cache.data });
  }

  // ⚠️ If cache exists but stale/unhealthy → serve + refresh in background
  if (cache.data && cache.data.length > 0) {
    rebuildCache().catch(console.error);
    return res.status(200).json({ products: cache.data });
  }

  // 🚀 No cache → build immediately
  if (!cache.data) {
    await rebuildCache();
